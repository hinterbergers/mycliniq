...
export function getServiceTypeOverrides(
  shiftPreferences?: unknown,
): ServiceType[] {
  if (!shiftPreferences || typeof shiftPreferences !== "object") return [];
  const overrides = (shiftPreferences as { serviceTypeOverrides?: unknown })
...
export function getServiceTypesForEmployee(
  ...
  const baseTypes = overrides.length
    ? overrides
    : getServiceTypesForRole(input.role, serviceLines);
  const filtered = input.canOverduty
    ? baseTypes
    : baseTypes.filter((type) => type !== OVERDUTY_KEY);
  return input.canOverduty
    ? Array.from(new Set([...filtered, OVERDUTY_KEY]))
    : filtered;
}

export function employeeDoesShifts(
  input: {
    takesShifts?: boolean | null;
    role?: string | null;
    shiftPreferences?: unknown;
