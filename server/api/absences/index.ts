...
const resolveRoleGroup = (
  role?: string | null,
): VacationVisibilityGroup | null => {
...
const canBypassVacationLock = (reqUser: Express.Request["user"]) => {
  ...
  if (
    reqUser.isAdmin ||
    reqUser.appRole === "Admin" ||
    reqUser.appRole === "Editor"
  )
    return true;
  return (
...
const LOCK_EXEMPT_REASONS = new Set([
  "Krankenstand",
  "Pflegeurlaub",
  "Quarant\u00e4ne",
]);

const isLockRestrictedReason = (reason: string) =>
  !LOCK_EXEMPT_REASONS.has(reason);
...
const ensureVacationEntitlement = async (
  ...
  for (const range of ranges) {
    const usedDays = await countVacationDaysForYear(
      employeeId,
      range.year,
      excludeAbsenceId,
    );
...
export function registerAbsenceRoutes(router: Router) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!req.user.isAdmin) {
        const allowedGroups = await getVisibilityGroupsForUser(
          req.user.employeeId,
        );
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const { employeeId, startDate, endDate, reason, notes, createdById } =
        req.body;
      ...
      if (createdById) {
        const [creator] = await db
          .select()
          .from(employees)
          .where(eq(employees.id, createdById));
        if (!creator) {
  ...
  router.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!canApproveVacation(req.user)) {
        return res
          .status(403)
          .json({ success: false, error: "Keine Berechtigung zur Freigabe" });
      }
      ...
      if (approvedById) {
        const [approver] = await db
          .select()
          .from(employees)
          .where(eq(employees.id, approvedById));
        if (!approver) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (existing.reason !== "Zeitausgleich") {
        return validationError(
          res,
          "Nur Zeitausgleich kann direkt beantwortet werden",
        );
      }
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (
        !canBypassVacationLock(req.user) &&
        isLockRestrictedReason(String(existing.reason))
      ) {
        const { lockFrom, lockUntil } = await getVacationLockRange();
        if (
          lockFrom &&
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!req.user.isAdmin && req.user.employeeId !== empId) {
        ...
        if (!group || !allowedGroups.includes(group)) {
          return res.status(403).json({
            success: false,
            error: "Keine Berechtigung fuer diese Abwesenheiten",
          });
        }
      }
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const absences = await db
        ...
        .from(plannedAbsences)
        .leftJoin(employees, eq(plannedAbsences.employeeId, employees.id))
        .where(and(eq(plannedAbsences.year, y), eq(plannedAbsences.month, m)));
      ...
      return ok(res, {
        ...
        summary: {
          ...
          genehmigt: visibleAbsences.filter((a) => a.status === "Genehmigt")
            .length,
          abgelehnt: visibleAbsences.filter((a) => a.status === "Abgelehnt")
            .length,
        },
      });
    }),
