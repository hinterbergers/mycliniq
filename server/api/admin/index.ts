...
async function ensurePermissionCatalog(): Promise<void> {
  ...
  const missing = DEFAULT_PERMISSION_CATALOG.filter(
    (perm) => !existingByKey.has(perm.key),
  );
  if (missing.length) {
  ...
  const updates = DEFAULT_PERMISSION_CATALOG.filter((perm) => {
    ...
    return (
      current && (current.label !== perm.label || current.scope !== perm.scope)
    );
  });
...
export function registerAdminRoutes(app: Express): void {
  ...
  router.get("/clinic", requireClinicAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error: "Fehler beim Abrufen der Klinik-Einstellungen",
      });
    }
  });
  ...
  router.put("/clinic", requireClinicAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error: "Fehler beim Aktualisieren der Klinik-Einstellungen",
      });
    }
  });
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        console.error(
          "[API] Error in GET /api/admin/users/:id/permissions:",
          error,
        );
        res.status(500).json({
          success: false,
  ...
  router.put(
    ...
    async (req, res) => {
      try {
        ...
        console.error(
          "[API] Error in PUT /api/admin/users/:id/permissions:",
          error,
        );
        res.status(500).json({
          success: false,
