...
export function registerAdminRoutes(app: Express): void {
  ...
  app.get("/api/me", authenticate, requireAuth, async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Abrufen der Benutzerdaten" });
    }
  });
  ...
  router.get("/clinic", requireClinicAdmin, async (req, res) => {
    try {
      if (!req.user?.clinicId) {
        return res
          .status(404)
          .json({ success: false, error: "Keine Klinik zugeordnet" });
      }
      ...
      if (!clinic) {
        return res
          .status(404)
          .json({ success: false, error: "Klinik nicht gefunden" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Abrufen der Klinik-Einstellungen" });
    }
  });
  ...
  router.put("/clinic", requireClinicAdmin, async (req, res) => {
    try {
      ...
      if (!name || !slug) {
        return res
          .status(400)
          .json({ success: false, error: "Name und Slug sind erforderlich" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Aktualisieren der Klinik-Einstellungen" });
    }
  });
  ...
  router.get("/users", requireTechnicalAdmin, async (_req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Abrufen der Benutzer" });
    }
  });
  ...
  router.get("/users/:id/permissions", requireTechnicalAdmin, async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Abrufen der Berechtigungen" });
    }
  });
  ...
  router.put("/users/:id/permissions", requireTechnicalAdmin, async (req, res) => {
    try {
      ...
      if (!departmentId) {
        return res
          .status(400)
          .json({ success: false, error: "Abteilungs-ID ist erforderlich" });
      }
      if (!Array.isArray(permissionKeys)) {
        return res
          .status(400)
          .json({ success: false, error: "permissionKeys muss ein Array sein" });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ success: false, error: "Keine Berechtigung für diese Abteilung" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Fehler beim Aktualisieren der Berechtigungen" });
    }
  });
