...
const createAssignmentSchema = z.object({
  ...
  (data) => Boolean(data.employeeId) || Boolean(data.note?.trim()) || data.isBlocked === true,
...
export function registerWeeklyPlanRoutes(router: Router) {
  ...
  router.post("/:id/assign",
    ...
    asyncHandler(async (req, res) => {
      const { id } = req.params;
      const planId = Number(id);
      const { roomId, weekday, employeeId, roleLabel, assignmentType, note, isBlocked } = req.body;
  ...
  router.patch("/assignments/:assignmentId",
    ...
    asyncHandler(async (req, res) => {
      ...
      const { employeeId, roleLabel, assignmentType, note, isBlocked } = req.body;
      ...
      const nextEmployeeId = employeeId === undefined ? existing.employeeId : employeeId;
      const nextNote = note === undefined ? existing.note : note?.trim() || null;
      const nextIsBlocked = isBlocked ?? existing.isBlocked;

      if (!nextEmployeeId && !nextNote && !nextIsBlocked) {
        return validationError(res, "Leere Zuweisung ist nicht erlaubt. Bitte löschen statt leeren.");
      }
