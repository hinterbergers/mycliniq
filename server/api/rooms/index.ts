...
export function registerRoomRoutes(router: Router) {
   ...
   */
  router.put("/:id/weekday-settings",
    validateParams(idParamSchema),
  ...
  router.put("/:id/competencies",
    ...
    asyncHandler(async (req, res) => {
      ...
      if (competencyList.length > 0) {
        const newRequirements = competencyList.map((req: z.infer<typeof competencyRequirementSchema>) => ({
          roomId,
        ...
        await db
          .insert(roomRequiredCompetencies)
   ...
   */
  router.get("/:id/weekday-settings",
    validateParams(idParamSchema),
    asyncHandler(async (req, res) => {
      const { id } = req.params;
      const roomId = Number(id);
