...
export function registerEmployeeRoutes(router: Router) {
  ...
  router.post("/", 
    ...
    asyncHandler(async (req, res) => {
      ...
      if (existingEmployee) {
        return validationError(res, "Ein Mitarbeiter mit dieser E-Mail existiert bereits");
      }
  ...
  router.put("/:id/preferences",
    ...
    asyncHandler(async (req, res) => {
      ...
      const shiftPreferences = {
        ...
        preferredOffDays: preferredOffDays ?? (existing.shiftPreferences as any)?.preferredOffDays ?? [],
        notesForPlanning: notesForPlanning ?? (existing.shiftPreferences as any)?.notesForPlanning ?? null
      };
  ...
  router.put("/:id/competencies",
    ...
    asyncHandler(async (req, res) => {
      ...
      const competencyNames = updatedCompetencies.map(c => c.name).filter(Boolean) as string[];
      await storage.updateEmployee(employeeId, {
