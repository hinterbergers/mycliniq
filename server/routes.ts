...
export async function registerRoutes(
  ...
  app.patch("/api/resources/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (!resource) {
        return res.status(import type { Express, Request, Response } from "express";
          ...
          export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
            /**
             * 1) Modular API routes (source of truth for:
             *    /api/me, /api/admin, /api/employees, /api/absences, /api/projects, etc.)
             */
            registerModularApiRoutes(app);
          
            /**
             * 2) AUTH (keep here for now; later you can move to server/api/auth)
             */
            app.post("/api/auth/login", async (req: Request, res: Response) => {
              try {
                const { email, password, rememberMe } = req.body;
            ...
            /**
             * IMPORTANT:
             * /api/me is the new canonical user endpoint (in server/api/admin/index.ts).
             * Deprecate /api/auth/me to avoid duplicates/confusion.
             */
            app.get("/api/auth/me", (_req: Request, res: Response) => {
              return res.status(410).json({ error: "Deprecated. Use GET /api/me" });
            });
            ...
            /**
             * 3) ROSTER (still legacy, keep here until you migrate into server/api/roster)
             */
            app.get("/api/roster/:year/:month", async (req: Request, res: Response) => {
              try {
                const year = parseInt(req.params.year);
            ...
            /**
             * 4) SHIFT-SWAPS / SETTINGS / WISHES / PLANNED-ABSENCES / KNOWLEDGE
             * Keep as-is (unchanged) — these are not in modular API yet.
             * (Paste your existing handlers here unchanged.)
             */
            ...
            return httpServer;
          }404).json({ error: "Resource not found" });
      }
      res.json(resource);
    } catch (error) {
